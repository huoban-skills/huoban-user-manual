...
**反缩水（硬规则）**：机检报错的合法修复只有三种——补证据（回走查、查 hac、查已有结果数据）、改成界面原词、补图。删正文消错、改触发词绕检查都不算修复，发现即返工。缺证据的正确顺序是先补查，实在验证不了才标 `[待确认]`，不许直接删掉了事。**改版/重跑任务**：旧版是覆盖基线，渲染必须传 `--baseline <旧版.md>`，小节和常见问题缩水机检不过。机检通过是底线不是完成标准，完成的标准是骨架小节全覆盖、自检清单全过。
...
5. **写 `<产出目录>/outline.md`**，记三样：纳入的表、纳入的页面、章节清单。**章节清单列到操作小节**：每章下面用缩进列表列出小节名（主单据的新建、核销、审批各自一节），机检按它逐条核对正文覆盖度——它是用户确认的覆盖合同，写完不许删小节、并小节。两轨骨架不同：
