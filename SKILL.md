...
4. **标注与脱敏只有一份标准**：截图怎么画标注框、怎么脱敏，全按 [references/walkthrough-guide.md](references/walkthrough-guide.md) 第五节执行，其他文件不另立规则。外部平台截图前先跟用户确认哪些信息要脱敏。
...
1. **需求对齐**，跟用户问清：
   ...
   - **数据来源**：这个区的数据是从零造的，还是从客户真实环境复制的？从零造的整册零打码；复制来的让用户指认哪些表/字段是真实数据，只对那部分打码。答案记进 outline.md，走查时照此执行，不逐图猜。

2. **现状盘点，表和页面都要盘**：`hac table list-tables --space-id <space_id>` 拉表清单；工作台、看板、报表页这类**页面不在表清单里**，从浏览器左侧导航看一遍（或用 hac 页面命令现查），把非表入口一并列出。两份清单一起展示给用户确认哪些纳入、哪些排除；确认后 `hac table +resolve-id --table <表名>` 解析成纯数字 table_id 列对照表。不落在任何工作区的内容（纯平台功能、外部平台）记下来，没有元数据可盘。

3. **定文档骨架，写进 `<产出目录>/outline.md`，让用户确认后才动手**。outline.md 记四样：纳入的表、纳入的页面、章节清单、数据来源口径（哪些要打码），它是阶段三的写作依据，也是骨架环节确实做过的凭证。两轨骨架不同：
