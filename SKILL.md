...
| 渲染预览 + 格式机检 | `python3 scripts/render.py <文档.md> --vocab <采集目录>/vocab.json`；改版任务加 `--baseline <旧版.md>` |
...
- 机检报错的合法修复只有三种：补证据（回走查、查 hac、查已有结果数据）、改成界面原词、补图。
- 删正文消错、改触发词绕检查都不算修复，发现即返工。缺证据先补查，实在验证不了才标 `[待确认]`。
- 改版/重跑任务把旧版当覆盖基线，渲染必须传 `--baseline <旧版.md>`，小节和常见问题缩水机检不过。
- 机检通过是底线不是完成标准；完成的标准是骨架小节全覆盖、自检清单全过。
...
5. **写 `<产出目录>/outline.md`**，记三样：纳入的表、纳入的页面、章节清单。**章节清单列到操作小节**——每章下面用缩进列表列出小节名（主单据的新建、核销、审批各自一节），格式固定为：
   ...
   机检按缩进列表逐条核对正文覆盖度（常见问题、注意事项等固定小节不用列）——它是用户确认的覆盖合同，写完不许删小节、并小节。两轨骨架不同：
...
5. **渲染 + 机检**：`render.py <文档.md> --vocab <采集目录>/vocab.json`，改版任务加 `--baseline <旧版.md>`。同目录出同名 .html。md 是源文件，人工改 md 后重跑；html 只当预览不手改。机检含格式检查、截图证据语义审计（界面原词、框↔正文、角标顺序、notes 点位）和覆盖度门禁（骨架小节、常见问题、基线对照），退出码非 0 就不算写完，不许口头宣布完成；修复报错按「反缩水」硬规则执行。`--vocab` 不传，正文里「」引的字段名是否生造就没人核对。
