...
- 正文里的菜单、按钮、页签、提示语只能用**浏览器实测到的界面原词**，凭证是 `browser.py shot` 自动落盘的 `.png.meta.json` 截图证据。
- hac 里的表名、字段名、自动化名称是内部配置名，不得当界面词写进正文。配置叫「收款核销」，界面按钮可能是「核销应收」。
- 没有证据的说法标 `[待确认]`，不许按常见页面模式脑补："列表右侧还能查看详情/审批"这类没亲眼见过的不写。
- 违反以上任一条，阶段三第 5 步的机检会拦下。
